...
console.log("sort",sort,"filter",filter)
  ...
  const handleFetch = () => {
    if(sort||filter){
      ...
      fetch(`http://localhost:8080/products?_page=${page}`)
      .then((res) => res.json())
      .then((res) => setData(res))
      .catch((err) => console.log(err))
    }
  ...
  return (
   ...
   <div className="filter">
      ...
      <div style={{display:"flex",gap:"40px",height:"50px",alignItems:"center"}}><button onClick={() => setPage(page-1)}> <FirstPageIcon/> </button><h1>{page}</h1><button onClick={() => setPage(page+1)}> <LastPageIcon/> </button></div>
    </div>
  )
}
